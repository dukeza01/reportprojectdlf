...
function _buildCoverHTML(typeLabel, qLabel, docNo, dateStr){
  return `
  <div style="font-family:'Sarabun',sans-serif;width:794px;height:1050px;background:#ffffff;color:${_INK};padding:0;position:relative;box-sizing:border-box">
    <div style="padding:36px 48px 0">
      <div style="display:flex;justify-content:space-between;font-size:12px;color:${_INK_SOFT};margin-bottom:26px">
        <span>${docNo}</span>
...
async function _htmlToCanvas(htmlStr) {
  ...
  wrap.style.cssText = 'position:fixed;left:-9999px;top:0;z-index:-1;background:#ffffff';
...
/* ─── วาดข้อความท้ายกระดาษเป็นรูปภาพ (jsPDF ไม่มีฟอนต์ไทยในตัว, doc.text() ใช้ไม่ได้กับภาษาไทย) ─── */
function _footerCanvasImg(leftText, centerText, rightText) {
  const W = 1588, H = 60;
  ...
  ctx.textAlign = 'left';
  ctx.fillText(leftText, 0, H/2);
  ctx.textAlign = 'center';
  ctx.fillText(centerText, W/2, H/2);
  ctx.textAlign = 'right';
  ctx.fillText(rightText, W, H/2);
  return c.toDataURL('image/png');
}

/* ─── Slice a tall canvas into A4-height chunks and add as jsPDF pages ─── */
/* เว้นพื้นที่ท้ายกระดาษ (~15mm) ไว้เสมอ สำหรับใส่เลขหน้า/ชื่อหน่วยงานทับภายหลัง ไม่ให้เนื้อหาชนท้ายกระดาษ */
async function _canvasToPdfPages(doc, canvas, isFirstPage) {
  const { jsPDF } = window.jspdf;
  const FOOTER_RESERVE_MM = 15;
  const CONTENT_H_MM = 297 - FOOTER_RESERVE_MM;
  const PAGE_W_PX = canvas.width;        // 794*2 = 1588px at scale 2
  const PAGE_H_PX = Math.round(PAGE_W_PX * (CONTENT_H_MM/210)); // เนื้อหาต่อหน้า (เว้นท้ายกระดาษ)
  ...
  while (offsetY < totalH) {
    ...
    doc.addImage(imgData, 'JPEG', 0, 0, 210, CONTENT_H_MM);
    firstSlice = false;
    isFirstPage = false;
    offsetY += PAGE_H_PX;
  }
...
async function generatePDF() {
  ...
  try {
    ...
    const docNo     = _genDocNumber();
    const dateStr   = _thaiDateNow();

    // ── 1. สร้างเนื้อหารายงานเป็นชุดหน้า HTML (กึ่งทางการ: ปก → สรุปผู้บริหาร → เนื้อหา → หน้าลงนาม) ──
    _pdfProgress('📄 กำลังจัดทำหน้าปกและเอกสารรับรอง...', 12);
    const blocks = [];
    blocks.push(_buildCoverHTML(typeLabel.replace(/^ฉบับ/,''), qLabel, docNo, dateStr));
    blocks.push(_buildExecSummaryHTML(fp, qLabel));

    _pdfProgress('📊 กำลังสร้างเนื้อหารายงาน...', 30);
    if (_pdfOptSelected === 1 || _pdfOptSelected === 3) {
      blocks.push(_buildOverviewHTML(fp, qLabel));
    }
    if (_pdfOptSelected === 2 || _pdfOptSelected === 3) {
      blocks.push(_buildProjectsHTML(fp, qLabel, sSel));
    }
    blocks.push(_buildSignatureHTML(dateStr));

    // ── 2. แปลงแต่ละส่วนเป็นภาพและประกอบเป็น PDF (แต่ละส่วนเริ่มหน้าใหม่เสมอ) ──
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF('p','mm','a4');
    let isFirstPage = true;
    for (let i=0;i<blocks.length;i++) {
      _pdfProgress(`🖼️ กำลังจัดหน้าเอกสาร (${i+1}/${blocks.length})...`, 30 + Math.round((i+1)/blocks.length*50));
      const canvas = await _htmlToCanvas(blocks[i]);
      await _canvasToPdfPages(doc, canvas, isFirstPage);
      isFirstPage = false;
    }

    // ── 3. ใส่เลขหน้า "หน้า x จาก y" และชื่อหน่วยงานที่ท้ายกระดาษทุกหน้า ──
    //     หมายเหตุ: jsPDF ไม่มีฟอนต์ไทยในตัว จึงวาดข้อความท้ายกระดาษเป็นภาพ (canvas) แทน doc.text()
    _pdfProgress('🔢 กำลังใส่เลขหน้าและท้ายกระดาษ...', 88);
    await document.fonts.ready;
    const totalPages = doc.internal.getNumberOfPages();
    const orgShort = _ORG_NAME_TH.replace(/ในพระบรมราชูปถัมภ์/,'').trim();
    for (let i=1;i<=totalPages;i++) {
      const footerImg = _footerCanvasImg(orgShort, `หน้า ${i} จาก ${totalPages}`, docNo);
      doc.setPage(i);
      doc.setDrawColor(201,206,216);
      doc.setLineWidth(0.15);
      doc.line(12, 284, 198, 284);
      doc.addImage(footerImg, 'PNG', 12, 285.5, 186, 6);
    }

    // ── 4. บันทึกไฟล์ ──
    _pdfProgress('💾 กำลังบันทึกไฟล์ PDF...', 96);
    doc.save(fileName);

    _pdfProgress('✅ สร้างรายงาน PDF สำเร็จ!', 100);
    setTimeout(() => {
      closePdfModal();
      showToast('✅ ดาวน์โหลดรายงาน PDF สำเร็จ · ' + fileName, 5000);
    }, 600);

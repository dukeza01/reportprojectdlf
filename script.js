      ...
      html += `
        <div class="pdf-project-block" style="background:${rowBg};border-left:4px solid ${color};border-right:1px solid #e4e7ed;border-bottom:1px solid #e4e7ed;padding:14px 18px">
          ...
          <div class="pdf-image-block">
            <div style="background:#eef2fd;color:#3b72f0;font-size:12px;font-weight:700;padding:5px 10px;border-radius:6px;margin-bottom:6px">📸 รูปภาพประกอบการดำเนินงาน (${p.images.length} รูป)</div>
...
async function generatePDF() {
  ...
  try {
    ...
    // ── 3. รอ font + รูปภาพโหลดเสร็จแล้วค่อย print ──
    _pdfProgress('⏳ รอ font และรูปภาพโหลด...', 80);
    printWin.onload = () => {
      printWin.document.fonts.ready.then(() => {
        // รอรูปภาพทุกรูปใน print window โหลดให้เสร็จก่อน (สำคัญมากสำหรับรูปจาก Cloudinary)
        const imgs = Array.from(printWin.document.images || []);
        const waitImgs = Promise.all(imgs.map(img => {
          if (img.complete) return Promise.resolve();
          return new Promise(res => {
            img.addEventListener('load', res, { once: true });
            img.addEventListener('error', res, { once: true }); // ไม่ให้รูปพังบล็อกการพิมพ์
          });
        }));
        // กันเหนียว ไม่รอเกิน 8 วิ เผื่อรูปโหลดช้า/เน็ตมีปัญหา
        Promise.race([waitImgs, new Promise(res => setTimeout(res, 8000))]).then(() => {
          setTimeout(() => {
            printWin.focus();
            printWin.print();
